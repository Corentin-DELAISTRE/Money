...
class _lambda{
 // Attributs :
    ...
    protected function addField($name, $type = null, $libelle = null, $link = null) {
        ...
        //      $libelle : libellé
        //      $link : cible (si pertinent, et si omis poyr un lien c'est le nom du champ)
    ...
    function table(){
        // Rôle : Récuperer le nom de la table de l'objet courant
        // Paramètres : Aucun
        // Retour : le nom de la table de l'objet courant

        // Le nom de la table est stocké dans l'attribut table
        return $this->table;
    }
    ...
    function getTarget($fieldName) {
        ...
        $nomClasse = $this->field($fieldName)->name();
   ...
   function listChamps() {
    ...
    function multiTargetsList($targets = []){
        // Rôle : lister les éléments d'une classe où des attributd valent des valeurs précises
    ...
    function objToTabJSON(){
        ...
        $tabJSON = ["id"=>$this->id(),"type"=>$this->table];
    ...
    function verifCompte($email,$mdp){
        ...
        $sql = "SELECT `id`, ". $this->listChamps() . " FROM `$this->table` WHERE `email` = :email"; 
    ...
    function createForm($tabChamps){
        // Rôle : Créer un template de formulaire html
        // Paramètres : Un tableau indexé avec pour index les libelles des champs et pour valeur le noms des champs
        // Retourne un template de formulaire
    }
    ...
    function sendMailHTML($exp,$adresseExp,$sujet,$to,$content){

        // Rôle : envoyer un email à une personne en utilisant un template php
        /* Paramètres : le nom de l'expediteur $exp
                        l'adresse mail de l'expediteur $adresseExp
                        le sujet $sujet
                        l'adresse mail du destinataire $to
    ...
    // METHODES GENERALES MAIS ASSOCIEES AU PROJET

    function listAllMembres($critere){
        // Rôle : générer une liste de membre quel que soit leur type de compte, selon un critère de recherche
        // Paramètres : $critere le critère de recherche
        // Retourne : une liste d'objets soit investisseur soit gestionnaire  

        // Initialisation de la liste à retourner

        $listeFinale = [];
        
        // Recuperation des gestionnaires et ajout dans la liste
        $gestionnaire = new gestionnaire();
        $listeGestionnaire = $gestionnaire->trierGestionnaire($_GET["critere"]);
        // Pour chaque gestionnaire, je l'ajoute à la liste à retourner
        foreach ($listeGestionnaire as $gest) {
            array_push($listeFinale,$gest);
        }
        // Recuperation des investisseurs
        
        $investisseur = new investisseur();
        $listeInvestisseur = $investisseur->trierInvestisseur($_GET["critere"]);
        // Pour chaque investisseur
        foreach ($listeInvestisseur as $invest) {
            array_push($listeFinale,$invest);
        }

        return $listeFinale;
    }

    function sendMailIdentifiantsHTML($exp,$adresseExp,$sujet,$to,$content,$clearMdp){

        // Rôle : envoyer un email à une personne pour lui fournir ses identifiants pour se connecter ainsi que le lien de connexion
        /* Paramètres : le nom de l'expediteur $exp
                        l'adresse mail de l'expediteur $adresseExp
                        le sujet $sujet
                        le destinataire $to
    ...
    function sendMailRefusHTML($exp,$adresseExp,$sujet,$to,$content,$raison){
        // Rôle : envoyer un email à une personne pour lui notifier les raisons du refus se sa proposition de projet
        /* Paramètres : le nom de l'expediteur $exp
                        l'adresse mail de l'expediteur $adresseExp
                        le sujet $sujet
                        l'adresse mail du destinataire $to
                        le contenu (le nom du template HTML) $content
                        la raisons du refus $raison
    ...
    function verifIfDes(){
        // Rôle : Verifier si le compte de l'utilisateur est désactivé où non
        // Paramètres : aucun puisqu'on va regarder sur l'utilisateur connecté
        // Retourne : true si désactivé sinon false

        // Récuperation su statut du compte de l'utilisateur
        $statut = session_userconnected()->get("statut");

        // Un compte désactivé sera traité comme un utilisateur non-connecté donc si jamais le statut est une chaine vide on retourne true
        if(empty($statut) or ($statut === "DES")){
            return true;
            // Sinon si le compte est actif on retourne false
        }else if($statut === "ACT"){
            return false;
        }
    }
